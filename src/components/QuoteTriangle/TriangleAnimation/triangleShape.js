...
import { MainCont, Sides, ShapeCont } from "./triangleStyle.js";
import "./triangleKeyframes.css";

const Triangle = (props) => {
  const { size, delay = 0, inView = true } = props;
  const WIDTH = size;
  ...
  return (
    <ShapeCont Width={WIDTH} Height={HEIGHT} id="Triange_ShapeCont">
      <Sides className="side1" />
      <Sides className="side2" />
      <Sides className="side3" />
    </ShapeCont>
  );
};
