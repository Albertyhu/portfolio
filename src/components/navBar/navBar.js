...
import { AppContext, NavBarContext } from "../../context/contextItem.js";
import RenderMobileMenu from "./mobileMenu.js";
import { useNavigate } from "react-router";
import { NavBar, ItemWrapper, Item, BurgerIcon } from "./myStyle.js";
...
const RenderNavBar = (props) => {
  ...
  useEffect(() => {
    if (isHomePage && VisibleBackground) setTextColor("#ffffff");
    else {
      setTextColor("#333333");
    }
  ...
  return (
    ...
    >
      <ItemWrapper id="NavbarWraper">
        <Item Color={textColor} onClick={GoHome}>
          Home
        </Item>
        <Item Color={textColor} onClick={GoProject}>
          Projects
        </Item>
        <Item Color={textColor} onClick={GoTestimonial}>
          Testimonials
        </Item>
        <Item Color={textColor} onClick={GoContact}>
          Contact Me
        </Item>
      </ItemWrapper>
    </NavBar>
...
const MobileMenu = () => {
  const { level, isHomePage, toggleMenu, VisibleBackground } =
    useContext(NavBarContext);
  ...
  const [onHero, setOnHero] = useState(
    function () {
